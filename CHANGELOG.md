...
- `public/banner/sceau-capot.svg` : compte bestiaire corrigé (`6+1` → `5+1`, `6 créatures` → `5 créatures`), description mise à jour — sous-titre déplacé du header vers la bottom legend (correction occlusion par ordre de peinture SVG header → médaillon) ; chaque flame-spark chevron alignée exactement sur le spoke-end
...
- `public/banner/sceau-capot.svg` : refonte complète de la section « Ce qu'Eldoria a sous le capot » — chaque médaillon gagne un **glyphe SVG distinctif** (crâne à yeux rouges pour BESTIAIRE, parchemin-rouleau & sceau de cire pour QUÊTES, mentor encapuchonné à œil lumineux & pointe de baguette pour MENTORS, coffre à cadenas avec étincelles pour COFFRES, fiole à potion devant enclume pour RECETTES, grimoire ouvert avec rune incandescente pour SORTS) ; l'**Arbre d'Argent central** redessiné en style Yggdrasil argenté (tronc gradient `url(#capBark)`, 5 branches sweepées, 5 grappes de feuilles crème `#fff4c2` avec highlights doré, cœur incandescent `url(#capTreeEmber)` à pulsation r 20→26 dur=2.4s, ruban doré `capRibbon` portant « 52+ », légendes italiques « entités tissées / L'ARBRE D'ARGENT / âme du royaume ») ; nouveau **header complet** (eyebrow `◆ SIX PILIERS — UN SEUL ROYAUME ◆`, titre principal `Ce qu'Eldoria a sous le capot`, sous-titre italique) ; **rayons améliorés** (stroke gradient `url(#capSpoke)` + **pointe de flamme** polygonale à chaque extrémité médaillon) ; nouveaux gradients `capBark`, `capTreeEmber`, `capBeastGlow`, `capSpoke` — chaque médaillon garde son glyphe SVG distinctif ; header compact (eyebrow + titre sans sous-titre pour éviter l'occlusion par ordre de peinture SVG header -> médaillon) ; sous-titre relocalisé dans la bottom legend ; flame-spark chevron alignée exactement sur chaque spoke-end
